...
async function cacheHTML5Applications(req) {
  const sapCloudServiceKeys = html5AppsCache.get(req.tenant);
  if (req.skipCachedApps || !sapCloudServiceKeys || Object.keys(sapCloudServiceKeys).length === 0) {
    const queryIndex = req.url.indexOf('?');
    const queries = queryIndex > -1 && req.url.indexOf('sap.cloud.service', queryIndex) < 0 ? req.url.slice(queryIndex) : '';
    ...
    subscribedApplications && subscribedApplications.length > 0 && _addHTML5ApplicationsToCache(subscribedApplications, req);
  }
}
...
function _addHTML5ApplicationsToCache(applicationsMetadata, req) {
  const html5ApplicationsCache = {};
  ...
  applicationsMetadata.forEach((app) => {
    ...
    if (app.configuration && credentials && (credentials.xsuaa || credentials.ias) && app.sapCloudService) {
      ...
      if (!html5ApplicationsCache[sapCloudServiceKey]) {
        ...
        html5ApplicationsCache[sapCloudServiceKey].credentials['html5-apps-repo'] = { app_host_id: app.appHostId};
